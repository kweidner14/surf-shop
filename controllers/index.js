...
module.exports = {
    ...
    async putForgotPw(req, res, next) {
        ...
        const msg = {
            to: user.email,
            from: 'Surf Shop Admin <codemode.io@gmail.com>',
            subject: 'Surf Shop - Forgot Password / Reset',
            text: `You are receiving this because you (or someone else) have requested the 
            reset of the password for your account.
			Please click on the following link, or copy and paste it into your browser to 
			complete the process:
			http://${req.headers.host}/reset/${token}
			If you did not request this, please ignore this email and your password will 
			remain unchanged.`.replace(/ {12}/g, ''), // .replace 12 empty spaces with none.
        };
    ...
    async putReset(req, res, next) {
        ...
        if(req.body.password === req.body.confirm) {
            ...
            await user.save();
            // 'login' lets us access req.login, allowing async/await
            const login = util.promisify(req.login.bind(req));
            await login(user);
        } else {
